...
void GPIO_init(void)
{
  ...
  gpio_af_set(TIMER_BLDC_BL_PORT, GPIO_AF_2, TIMER_BLDC_BL_PIN);
	...
	// Init SPEED_PWM INPUT
	gpio_mode_set(SPEED_PWM_PORT , GPIO_MODE_INPUT,GPIO_PUPD_PULLUP, SPEED_PWM_PIN);	

	// Init STEER_PWM INPUT
	gpio_mode_set(STEER_PWM_PORT , GPIO_MODE_INPUT,GPIO_PUPD_PULLUP, STEER_PWM_PIN);	

// Init PANIC BUTTON INPUT
	gpio_mode_set(PANIC_BUTTON_PORT , GPIO_MODE_INPUT,GPIO_PUPD_NONE, PANIC_BUTTON_PIN);	

// Init PANIC RAIN SENSOR INPUT
	gpio_mode_set(RAIN_SENSOR_PORT , GPIO_MODE_INPUT,GPIO_PUPD_NONE, RAIN_SENSOR_PIN);	


	// Init ACTUATOR PWM OUTPUT  //PRIMA DI SCOMMENTARE DEVO LIBERARE IL RED_LED
	//gpio_mode_set(ACTUATOR_PORT , GPIO_MODE_OUTPUT, GPIO_PUPD_NONE, ACTUATOR_PIN);	
	//gpio_output_options_set(ACTUATOR_PORT, GPIO_OTYPE_PP, GPIO_OSPEED_2MHZ, ACTUATOR_PIN);
