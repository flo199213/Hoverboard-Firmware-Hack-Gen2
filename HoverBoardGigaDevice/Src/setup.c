...
#ifdef MASTER
uint8_t usartSteer_rx_buf[USART_STEER_RX_BUFFERSIZE];
#endif
...
#ifdef MASTER
	// Init USART0
	gpio_mode_set(USART_STEER_TX_PORT , GPIO_MODE_AF, GPIO_PUPD_PULLUP, USART_STEER_TX_PIN);	
	gpio_mode_set(USART_STEER_RX_PORT , GPIO_MODE_AF, GPIO_PUPD_PULLUP, USART_STEER_RX_PIN);
	gpio_output_options_set(USART_STEER_TX_PORT, GPIO_OTYPE_PP, GPIO_OSPEED_50MHZ, USART_STEER_TX_PIN);
	gpio_output_options_set(USART_STEER_RX_PORT, GPIO_OTYPE_PP, GPIO_OSPEED_50MHZ, USART_STEER_RX_PIN);	
	gpio_af_set(USART_STEER_TX_PORT, GPIO_AF_0, USART_STEER_TX_PIN);
	gpio_af_set(USART_STEER_RX_PORT, GPIO_AF_0, USART_STEER_RX_PIN);
...
void USART_Steer_init(void)
{
	...
	usart_baudrate_set(USART_STEER, 19200);
	usart_parity_config(USART_STEER, USART_PM_NONE);
	usart_word_length_set(USART_STEER, USART_WL_8BIT);
	usart_stop_bit_set(USART_STEER, USART_STB_1BIT);
	usart_oversample_config(USART_STEER, USART_OVSMOD_16);
	
	// Enable both transmitter and receiver
	usart_transmit_config(USART_STEER, USART_TRANSMIT_ENABLE);
	usart_receive_config(USART_STEER, USART_RECEIVE_ENABLE);
	
	// Enable USART
	usart_enable(USART_STEER);
	
	// Interrupt channel 1/2 enable
	nvic_irq_enable(DMA_Channel1_2_IRQn, 2, 0);
	
	// Initialize DMA channel 2 for USART_STEER RX
	dma_deinit(DMA_CH2);
	dma_init_struct_usart.direction = DMA_PERIPHERAL_TO_MEMORY;
	dma_init_struct_usart.memory_addr = (uint32_t)usartSteer_rx_buf;
	dma_init_struct_usart.memory_inc = DMA_MEMORY_INCREASE_ENABLE;
	dma_init_struct_usart.memory_width = DMA_MEMORY_WIDTH_8BIT;
	dma_init_struct_usart.number = USART_STEER_RX_BUFFERSIZE;
	dma_init_struct_usart.periph_addr = USART_STEER_DATA_RX_ADDRESS;
	dma_init_struct_usart.periph_inc = DMA_PERIPH_INCREASE_DISABLE;
	dma_init_struct_usart.periph_width = DMA_PERIPHERAL_WIDTH_8BIT;
	dma_init_struct_usart.priority = DMA_PRIORITY_ULTRA_HIGH;
	...
	// USART DMA enable for transmission and receive
	usart_dma_receive_config(USART_STEER, USART_DENR_ENABLE);
