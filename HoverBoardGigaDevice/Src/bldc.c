...
//----------------------------------------------------------------------------
// Set pwm -1000 to 1000, field weakening 0 to 1000
//----------------------------------------------------------------------------
void SetPWM(int16_t setPwm, int16_t setWeak)
{
	bldc_inputFilterPwm = CLAMP(setPwm, -1000, 1000);
	bldc_weak = CLAMP(setWeak, 0, 1000);
}
...
void CalculateBLDC(void)
{
	...
	int weaky = 0; // yellow = phase A
	int weakb = 0; // blue   = phase B
	int weakg = 0; // green  = phase C
  ...
  if (bldc_outputFilterPwm > 0)
	{
		// Set field weakening 5 positions up-front (behind current position)
    blockPWM(bldc_weak, (pos+5) % 6, &weaky, &weakb, &weakg);
  }
	else
	{
		// Set field weakening 1 positions up-front (in front of current position)
    blockPWM(-bldc_weak, (pos+1) % 6, &weaky, &weakb, &weakg);
  }
