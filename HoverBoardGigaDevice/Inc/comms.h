...
#ifdef MASTER
// Steering device struct
typedef struct
{
	float v_batt;
	float current_dc_Master;
	float current_dc_Slave;
	uint8_t realSpeed;
	uint8_t activateWeakening;
	uint8_t beepsBackwards;
	uint8_t mosfetOutSlave;
	uint8_t lowerLEDSlave;
	uint8_t upperLEDSlave;
	uint8_t mosfetOutMaster;
	uint8_t lowerLEDMaster;
	uint8_t upperLEDMaster;
} steering_device_TX_buffer;
